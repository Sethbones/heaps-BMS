...
class Matrix {
	...
	**/
	...
	/**
		Returns a new Vector2 that is a result of transforming the parameter `Vec2` by this Matrix.
	**/
	public inline function transform( vec2 : Vector2 ) {
		return new Vector2(vec2.x * a + vec2.y * c + x, vec2.x * b + vec2.y * d + y);
	}
	...
	**/
	...
	/**
		uhh there's no option to provide a Vector, devs be trippin.

		Returns a Vector2 with a total scaling applied by the Matrix.
		@param p Optional Vector2 instance. If provided, sets values of the given Vector2 and returns it. Otherwise returns a new Vector2 instance.
	**/
	...
	**/
	...
	/**
		Returns a Vector2 with the `x` and `y` of the Matrix.
		@param p Optional Vector2 instance to use. Otherwise returns new instance.
	**/
	public inline function getPosition( ?p : Vector2 ) {
		if( p == null ) p = new Vector2();
		p.set(x,y);
		return p;
	}
