...
class Circle extends Collider {
	...
	**/
	public inline function lineIntersect(p1 : h2d.col.old.Point, p2:h2d.col.old.Point) : Array<Point> {
		var dx = p2.x - p1.x;
		var dy = p2.y - p1.y;
		var a = dx * dx + dy * dy;
		...
		if(d == 0) {
			var t = -b / (2 * a);
			return [new h2d.col.old.Point(p1.x + t * dx, p1.y + t * dy)];
		}

		var t1 = (-b - Math.sqrt(d)) / (2 * a);
		var t2 = (-b + Math.sqrt(d)) / (2 * a);
		return [new h2d.col.old.Point(p1.x + t1 * dx, p1.y + t1 * dy), new h2d.col.old.Point(p1.x + t2 * dx, p1.y + t2 * dy)];
	}
