package h2d.col.old;
import hxd.Math;
//this is an abstract because its a point array disguised as a class, that's interesting
/**
 * in general, from reading everything here, this seems like an "ultimate solution" to collision,
 * as it is by far the most complete collision file in the entire engine.
 * 
 * and not only that there's several functions here that on the surface make no sense,
 * like at what point are you going to need a macro function to draw a circle,
 * but in context to every other collision shape being only half finished it makes sense, because this was likely meant to replace all of them
 * 
 * there's one thing however i don't agree with, splitting it into 2 files, which was likely done because polygon is not a shape but an array of points
 * i think it should be self contained, makes it easier to read and make changes to
*/
/**
...
abstract Polygon(Array<Point>) from Array<Point> to Array<Point> {
	...
	**/
	...
	//i have no idea if this used to do something, but it doesn't now
	/**
	...
	**/
	public function centroid() {
		...
		return new h2d.col.old.Point(cx, cy);
	}
	...
	**/
	public function transform(mat: h2d.col.old.Matrix) {
	...
	**/
	public function transformed(mat: h2d.col.old.Matrix) {
		var ret = points.copy();
	...
	**/
	public function findClosestPoint(pt : h2d.col.old.Point, maxDist : Float) {
		var closest = null;
	...
	**/
	public function projectPoint(pt: h2d.col.old.Point, ?out : h2d.col.old.Point) {
		var p1 = points[points.length - 1];
		var closest = new h2d.col.old.Point();
		if (out == null) out = new h2d.col.old.Point();
		var minDistSq = 1e10;
		for(p2 in points) {
	...
	**/
	...
	public function rayIntersection( r : h2d.col.old.Ray, bestMatch : Bool, ?oriented = false ) : Float {
		var dmin = -1.;
		var p0 = points[points.length - 1];
		...
		for(p in points) {
			...
			var v = new h2d.col.old.Point(x - r.px, y - r.py);
	...
	// 1 --> Clockwise
	// 2 --> Counterclockwise
	inline function orientation(p : h2d.col.old.Point, q : h2d.col.old.Point, r : h2d.col.old.Point) {
		var v = side(p, q, r);
	...
	**/
	inline function onSegment(p : h2d.col.old.Point, q : h2d.col.old.Point, r : h2d.col.old.Point) {
	...
	**/
	function intersect(p1 : h2d.col.old.Point, q1 : h2d.col.old.Point, p2 : h2d.col.old.Point, q2 : h2d.col.old.Point) {
	...
	**/
	function getIntersectionPoint(a : h2d.col.old.Point, b : h2d.col.old.Point, c : h2d.col.old.Point, d : h2d.col.old.Point) : h2d.col.old.Point {
        ...
        return new h2d.col.old.Point(x, y);
	}
