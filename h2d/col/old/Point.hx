...
class PointImpl #if apicheck implements h2d.impl.PointApi<Point,Matrix> #end {
	...
	**/
	public inline function normalized() {//this is like the first time i ever encountered epsilon2, the usage here makes sense, its to account for the float being a float
		var k = lengthSq();
		if( k < Math.EPSILON2 ) k = 0 else k = Math.invSqrt(k);
		return new h2d.col.old.Point(x*k,y*k);
	}
	...
	**/
	public inline function load( p : h2d.col.old.Point ) {
		this.x = p.x;
		this.y = p.y;
	}
